...
def read_audio(ws, timeout):
    ...
    resp = assistant.message_stateless(
    assistant_id='036bd9b1-f69a-4e32-9221-ea16fb0d252a',
    input={
        'message_type': 'text',
        'text':  file1.read()
...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        print(data['results'][0]['alternatives'][0]['transcript'][::-1])
        
        file1 = open("output.txt","w",encoding='utf-8')#write mode
        file1.write(data['results'][0]['alternatives'][0]['transcript'])
        file1.close()
